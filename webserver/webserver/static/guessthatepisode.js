$(function() {    
    async function loadEpisodeData() {
        return await $.getJSON("/episodes");
    };

    loadEpisodeData().then(function(d) {
        let data = d;
        let timer = 60;
        let points = 3;
        let sample = 0;
        ...
        const maxPoints = 525//data.length*3; 
        ...
        changeAudioSrc = function() {
            let key = Object.keys(episode);
            //console.log("CHANGING AUDIO SOURCE");
            console.log(episode[key]);
            $("#audio").attr("src", "/static/samples/" + key);
        };
